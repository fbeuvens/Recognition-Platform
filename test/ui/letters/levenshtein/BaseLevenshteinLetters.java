package test.ui.letters.levenshtein;
...
public class BaseLevenshteinLetters {
	...
	public BaseLevenshteinLetters(){
		...
		tw = new TraceWriter("UILettersLevenshtein.txt");
				...
				for(int characterIndex = 0; characterIndex < 26; characterIndex++){
					...
					for(int teIndex = tmp; teIndex != firstTeIndex ; teIndex = (teIndex%10)+1){
						...
						for(int otherUser = 1; otherUser <= 5; otherUser++)
							if(otherUser != user){
								String[] foundChars =  lev.recognizeForAllKnn(strings.get(""+otherUser+Utils.intToChar(characterIndex)+teIndex),normalisation,knn);
