package test.ui.digits.oneDollar;
...
public class BaseOneDollarDigits {
	...
	public BaseOneDollarDigits(){
		...
		tw = new TraceWriter("UIDigitsOneDollar.txt");
		...
		for(int firstTeIndex = 1; firstTeIndex <= 10; firstTeIndex++){
			...
			for(int digitIndex = 0; digitIndex < 10; digitIndex++){
				...
				for(int teIndex = firstTeIndex; mod*10+teIndex < firstTeIndex+numberTrainingExamples /*&& (teIndex != firstTeIndex || firstTeIndexPassage)*/; teIndex++){
					recognizer.AddTemplate(Utils.intToDigit(digitIndex)+"", points.get(""+user+Utils.intToDigit(digitIndex)+teIndex));
				...
				for(int digitIndex = 0; digitIndex < 10; digitIndex++){
					...
					for(int teIndex = tmp; teIndex != firstTeIndex ; teIndex = (teIndex%10)+1){
						...
						for(int otherUser = 1; otherUser <= 5; otherUser++)
							if(otherUser != user){
								ArrayList<Point> toRecognize = points.get(""+otherUser+Utils.intToDigit(digitIndex)+teIndex);
								if(toRecognize.size() > 0){
									String[] foundDigits =  recognizer.RecognizeForAllKnn(toRecognize,knn);
									for(int i = 0; i < foundDigits.length; i++){
										String foundDigit = foundDigits[i];
										//String foundDigit = recognizer.RecognizeKnn(toRecognize,knn);
			...
			System.out.println("digits 1$: nte="+numberTrainingExamples+"|user="+user+"|ftei="+firstTeIndex+" recognized");
		}
		for(int knn = 0; knn < numberTrainingExamples; knn++){
