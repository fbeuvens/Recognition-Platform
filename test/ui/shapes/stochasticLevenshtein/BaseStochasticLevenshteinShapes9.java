package test.ui.shapes.stochasticLevenshtein;
...
public class BaseStochasticLevenshteinShapes9 {
	...
	public BaseStochasticLevenshteinShapes9(){
	...
	public void UI(){
		for(int normalisation = 0; normalisation <= 3; normalisation++){
			tw = new TraceWriter("UIShapesStochasticLevenshtein9Norm"+normalisation+".txt");
			tw.println("-------------------------------------------------- User Dependent -------------------------------------------------");
			tw.println();
			for(int numberTrainingExamples = 9; numberTrainingExamples <= 9; numberTrainingExamples++){
		...
		for(int firstTeIndex = 1; firstTeIndex <= 10; firstTeIndex++){
			...
			for(int gestureIndex = 0; gestureIndex < 8; gestureIndex++){
				...
				for(int teIndex = firstTeIndex; mod*10+teIndex < firstTeIndex+numberTrainingExamples /*&& (teIndex != firstTeIndex || firstTeIndexPassage)*/; teIndex++){
					cr.addTemplate(Utils.intToShape(gestureIndex)+"", strings.get(""+user+Utils.intToShape(gestureIndex)+teIndex));
				...
				for(int gestureIndex = 0; gestureIndex < 8; gestureIndex++){
					...
					for(int teIndex = tmp; teIndex != firstTeIndex ; teIndex = (teIndex%10)+1){
						...
						for(int otherUser = 1; otherUser <= 5; otherUser++)
							if(otherUser != user){
								String[] foundShapes =  cr.recognizeForAllKnn(strings.get(""+otherUser+Utils.intToShape(gestureIndex)+teIndex),normalisation,knn);
								for(int i = 0; i < foundShapes.length; i++){
									String foundDigit = foundShapes[i];
									confusionMatrixes.get(i)[gestureIndex][Utils.shapeToInt(foundDigit)]++;
									userConfusionMatrixes.get(i)[gestureIndex][Utils.shapeToInt(foundDigit)]++;
								}
							}
						/*if(teIndex == tmp)
							firstTeIndexPassage = false;
						i++;*/
						//System.out.println(""+firstTeIndex+Utils.intToShape(gestureIndex)+teIndex+" recognized");
					}
					//System.out.println(i);
					System.out.println("norm="+normalisation+"|nte="+numberTrainingExamples+"|user="+user+"|ftei="+firstTeIndex+"|char="+Utils.intToShape(gestureIndex)+" recognized");
				}
			}
	...
	public static void main(String[] args){
		BaseStochasticLevenshteinShapes9 bsll = new BaseStochasticLevenshteinShapes9();
		bsll.UI();
