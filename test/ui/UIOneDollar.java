...
public class UIOneDollar {
	...
	public UIOneDollar(int category){
		this.category = category;
		confusionMatrixes = new ArrayList<int[][]>();
		points = new Hashtable<String, ArrayList <Point>>();
		switch(category){
		case 1: tw = new TraceWriter("digits/ui/UIDigitsOneDollar.txt");break;
		case 2: tw = new TraceWriter("gestures/ui/UIGesturesOneDollar.txt");break;
		case 3: tw = new TraceWriter("letters/ui/UILettersOneDollar.txt");break;
		case 4: tw = new TraceWriter("shapes/ui/UIShapesOneDollar.txt");break;
		}
	...
	public void UI(int user, int numberTrainingExamples){
		...
		for(int firstTeIndex = 1; firstTeIndex <= 10; firstTeIndex++){
				...
				for(int gestureIndex = 0; gestureIndex < Utils.maxGestureIndex(category); gestureIndex++){
					...
					for(int teIndex = tmp; teIndex != firstTeIndex ; teIndex = (teIndex%10)+1){
						for(int otherUser = 1; otherUser <= 5; otherUser++)
							if(otherUser != user){
								ArrayList<Point> toRecognize = points.get(""+otherUser+Utils.intToGesture(gestureIndex,category)+teIndex);
			...
			System.out.println("ui 1$: nte="+numberTrainingExamples+"|user="+user+"|ftei="+firstTeIndex+" recognized");
		}
		for(int knn = 0; knn < numberTrainingExamples; knn++){
	...
	public static void main(String[] args){
		...
		UIOneDollar uiod = new UIOneDollar(category);
		uiod.UI();
	}
