...
public class BaseStochasticLevenshteinLetters {
	...
	public BaseStochasticLevenshteinLetters(){
		...
		for(int i = 15; i <= 15; i+=3){
			...
			for(int j = 0; j < 8; j++){
				for(int k = 12; k <= 14; k++){
					//if(!(k >= i && k <i+3)){
						for(int l = 1; l <= 1; l++){
							cr.addTemplate(Utils.intToChar(j)+"", strings.get(""+k+Utils.intToChar(j)+l));
						}
					//}
				}
			}
			...
			for(int j = i; j < i+1; j++){
				for(int k = 0; k < 8; k++){
					for(int l = 1; l <= 10; l++){
						char foundChar = cr.recognize(strings.get(""+j+Utils.intToChar(k)+l)).charAt(0);
						confusionMatrixes.get(0)[k][Utils.charToInt(foundChar)]++;
						System.out.println(""+j+Utils.intToChar(k)+l+" recognized");
					}
				}
			}
		}
	...
	public void UD(){
		for(int normalisation = 0; normalisation <= 3; normalisation++){
			tw = new TraceWriter("StochasticLevenshteinLettersNorm"+normalisation+".txt");
	...
	public void UD(int user){
		...
		for(int i = 0; i < 10; i++)
			for(int j = 1; j <= 2; j++)
				cr.addTemplate(Utils.intToChar(i)+"", strings.get(""+user+Utils.intToChar(i)+j));
		
		cr.compile();
		for(int i = 0; i < 10; i++)
			for(int j = 3; j <= 10; j++){
				char foundChar = cr.recognize(strings.get(""+user+Utils.intToChar(i)+j)).charAt(0);
				confusionMatrixes.get(0)[i][Utils.charToInt(foundChar)]++;
				System.out.println(""+user+Utils.intToChar(i)+j+" recognized");
			}
	}
	
	public static void main(String[] args){
		BaseStochasticLevenshteinLetters bsll = new BaseStochasticLevenshteinLetters();
		bsll.UD();
