package test.ud.digits.oneDollar;
...
public class BaseOneDollarDigits {
	...
	public BaseOneDollarDigits(){
		...
		tw = new TraceWriter("UDDigitsOneDollar.txt");
			...
			for(int knn = numberTrainingExamples; knn <= numberTrainingExamples; knn++){
				for(int digitIndex = 0; digitIndex < 10; digitIndex++){
					...
					for(int teIndex = tmp; teIndex != firstTeIndex ; teIndex = (teIndex%10)+1){
						...
						ArrayList<Point> toRecognize = points.get(""+user+Utils.intToDigit(digitIndex)+teIndex);
