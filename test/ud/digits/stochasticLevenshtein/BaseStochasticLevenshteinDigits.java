...
public class BaseStochasticLevenshteinDigits {
	...
	public BaseStochasticLevenshteinDigits(){
		...
		for(int i = 15; i <= 15; i+=3){
			...
			for(int j = 0; j < 8; j++){
				for(int k = 12; k <= 14; k++){
					//if(!(k >= i && k <i+3)){
						for(int l = 1; l <= 1; l++){
							cr.addTemplate(Utils.intToDigit(j)+"", strings.get(""+k+Utils.intToDigit(j)+l));
						}
					//}
				}
			}
			...
			for(int j = i; j < i+1; j++){
				for(int k = 0; k < 8; k++){
					for(int l = 1; l <= 10; l++){
						char foundChar = cr.recognize(strings.get(""+j+Utils.intToDigit(k)+l)).charAt(0);
						confusionMatrixes.get(0)[k][Utils.digitToInt(foundChar)]++;
						System.out.println(""+j+Utils.intToDigit(k)+l+" recognized");
					}
				}
			}
		}
	...
	public void UD(){
		for(int normalisation = 0; normalisation <= 3; normalisation++){
			tw = new TraceWriter("StochasticLevenshteinDigitsNorm"+normalisation+".txt");
		...
		for(int firstTeIndex = 1; firstTeIndex <= 10; firstTeIndex++){
			...
			for(int characterIndex = 0; characterIndex < 26; characterIndex++){
				...
				for(int teIndex = firstTeIndex; mod*10+teIndex < firstTeIndex+numberTrainingExamples /*&& (teIndex != firstTeIndex || firstTeIndexPassage)*/; teIndex++){
					cr.addTemplate(Utils.intToDigit(characterIndex)+"", strings.get(""+user+Utils.intToDigit(characterIndex)+teIndex));
			...
			for(int knn = numberTrainingExamples; knn <= numberTrainingExamples; knn++){
				for(int characterIndex = 0; characterIndex < 26; characterIndex++){
					...
					for(int teIndex = tmp; teIndex != firstTeIndex ; teIndex = (teIndex%10)+1){
						...
						String[] foundChars =  cr.recognizeForAllKnn(strings.get(""+user+Utils.intToDigit(characterIndex)+teIndex),normalisation,knn);
						for(int i = 0; i < foundChars.length; i++){
							char foundChar = foundChars[i].charAt(0);
							confusionMatrixes.get(i)[characterIndex][Utils.digitToInt(foundChar)]++;
							userConfusionMatrixes.get(i)[characterIndex][Utils.digitToInt(foundChar)]++;
						}
						/*if(teIndex == tmp)
							firstTeIndexPassage = false;
						i++;*/
						//System.out.println(""+firstTeIndex+Utils.intToDigit(characterIndex)+teIndex+" recognized");
					}
					//System.out.println(i);
					System.out.println("norm="+normalisation+"|nte="+numberTrainingExamples+"|user="+user+"|ftei="+firstTeIndex+"|char="+Utils.intToDigit(characterIndex)+" recognized");
				}
			}
	...
	public void UD(int user){
		...
		for(int i = 0; i < 10; i++)
			for(int j = 1; j <= 2; j++)
				cr.addTemplate(Utils.intToDigit(i)+"", strings.get(""+user+Utils.intToDigit(i)+j));
		
		cr.compile();
		for(int i = 0; i < 10; i++)
			for(int j = 3; j <= 10; j++){
				char foundChar = cr.recognize(strings.get(""+user+Utils.intToDigit(i)+j)).charAt(0);
				confusionMatrixes.get(0)[i][Utils.digitToInt(foundChar)]++;
				System.out.println(""+user+Utils.intToDigit(i)+j+" recognized");
			}
	}
	
	public static void main(String[] args){
		BaseStochasticLevenshteinDigits bsll = new BaseStochasticLevenshteinDigits();
		bsll.UD();
