...
public class DifferenceAngleLevenshteinDigits {
		...
		for(int i = 1; i <= 5; i++){
			for(int j = 0; j < 10; j++)
				for(int k = 1; k <= 10; k++){
					...
					strings.put(""+i+Utils.intToDigit(j)+k, algorithm.levenshtein.Utils.transformAngleDifference(new RecordParser("records/"+i+"/"+Utils.intToDigit(j)+"/"+Utils.intToDigit(j)+k+".txt").parse(),false));
				}
	...
	public static void main(String[] args){
		DifferenceAngleLevenshteinDigits bll = new DifferenceAngleLevenshteinDigits();
		bll.UD();
