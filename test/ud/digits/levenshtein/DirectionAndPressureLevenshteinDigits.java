...
public class DirectionAndPressureLevenshteinDigits {
	...
	public DirectionAndPressureLevenshteinDigits(){
		confusionMatrixes = new ArrayList<int[][]>();
		userRecognitionRates = new double[10][9];
		strings = new Hashtable<String, String>();
		tw = new TraceWriter("UDDigitsPressureDirectionAndLevenshtein.txt");
		...
		for(int i = 1; i <= 5; i++){
			for(int j = 0; j < 10; j++)
				for(int k = 1; k <= 10; k++){
					...
					strings.put(""+i+Utils.intToDigit(j)+k, algorithm.levenshtein.Utils.transformDirectionAndPressure(new RecordParser("records/"+i+"/"+Utils.intToDigit(j)+"/"+Utils.intToDigit(j)+k+".txt").parse(),false));
				}
	...
	public static void main(String[] args){
		DirectionAndPressureLevenshteinDigits bll = new DirectionAndPressureLevenshteinDigits();
		bll.UD();
