...
public class ResampleProcessedLevenshteinDigits {
		...
		for(int i = 1; i <= 5; i++){
			for(int j = 0; j < 10; j++)
				for(int k = 1; k <= 10; k++){
					...
					strings.put(""+i+Utils.intToDigit(j)+k, algorithm.levenshtein.Utils.transformWithProcessing(new RecordParser("records/"+i+"/"+Utils.intToDigit(j)+"/"+Utils.intToDigit(j)+k+".txt").parse(),true,false,false,false));
				}
	...
	public static void main(String[] args){
		ResampleProcessedLevenshteinDigits rpld = new ResampleProcessedLevenshteinDigits();
		rpld.UD();
