package test.ui2.gestures.levenshtein;
...
public class BaseLevenshteinGestures {
	...
	public BaseLevenshteinGestures(){
		...
		tw = new TraceWriter("UI2GesturesLevenshtein.txt");
		...
		for(int firstUserIndex = 1; firstUserIndex <= 10; firstUserIndex++){
			...
			for(int gestureIndex = 0; gestureIndex < 16; gestureIndex++){
				...
				for(int otherUser = firstUserIndex; mod*10+otherUser < threshold  ; otherUser++){
					...
					lev.addTemplate(Utils.intToGest(gestureIndex)+"", strings.get(""+otherUser+Utils.intToGest(gestureIndex)+"1"));
			...
			for(int knn = numberTrainingExamples; knn <= numberTrainingExamples; knn++){
				...
				System.out.println("gestures lev: norm="+normalisation+"|nte="+numberTrainingExamples+"|user="+user+"|fui="+firstUserIndex+" recognized");
			}
		}
		for(int knn = 0; knn < numberTrainingExamples; knn++)
